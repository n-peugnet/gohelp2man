...
//go:generate go run . -version-string=v1.0.0 -include=gohelp2man.h2m -output=gohelp2man.1 ./gohelp2man
...
// currentVersion returns the current version of gohelp2man as found in build
// info, or else, the value of the "main.version" variable (which can be set
// via -ldflags).
...
func main() {
	...
	if flagVersion {
		fmt.Println(Name, currentVersion())
		os.Exit(0)
	}
